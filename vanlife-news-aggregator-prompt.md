...
### Первый запуск (создание проекта с нуля)
```
Изучи vanlife-news-aggregator-prompt.md в корне репозитория.
Это мастер-промт для разработки новостного агрегатора.
Создай структуру проекта и начни с PHASE 1, задача 1.1.
После каждой задачи обновляй PROGRESS.md и README.md.
```

### Продолжение работы (новая сессия)
```
Изучи vanlife-news-aggregator-prompt.md и PROGRESS.md.
Определи текущую задачу и продолжи разработку с того места, где остановились.
Не забывай обновлять PROGRESS.md и README.md после каждой задачи.
```

### Если ИИ сбился или делает не то
```
СТОП. Вернись к vanlife-news-aggregator-prompt.md.
Проверь PROGRESS.md — какая задача текущая?
Продолжи строго по плану, не отклоняйся.
```

### Запрос статуса
```
Покажи текущий статус проекта:
1. Какая фаза и задача сейчас?
2. Что уже сделано (из PROGRESS.md)?
3. Что следующее по плану?
```

### Завершение сессии
```
Перед завершением:
1. Обнови PROGRESS.md с текущим статусом
2. Обнови README.md если были изменения
3. Сделай коммит всех изменений
4. Напиши краткое резюме что сделано
```
...
3. **Не переходи к следующей Фазе, пока все КРИТЕРИИ ПРИЁМКИ (Acceptance Criteria) текущей Фазы не будут ФУНКЦИОНАЛЬНО подтверждены (через БД или логи).**
...
## 📅 ЭТАПЫ РАЗРАБОТКИ (с КРИТЕРИЯМИ ПРИЕМКИ)
...
1. Скрипт `scripts/fetch_news.php` запускается без фатальных ошибок.
2. В таблице `articles` есть **минимум 30 статей** со статусом `new`.
3. Файл `NewsFetcher.php` использует работающий механизм получения URL (через декодер или временный fallback), `original_url` не пустой.

---

### PHASE 2: ИИ-обработка (ТЕКУЩИЙ ПРИОРИТЕТ)
**Цель:** Новости переводятся, категоризируются, фильтруются.

**Задачи:**
- [ ] 2.1 OpenAI Provider (интерфейс + реализация)
- [ ] 2.2 NewsProcessor — оценка релевантности и проверка на "опасный" контент
- [ ] 2.3 NewsProcessor — категоризация и тегирование
- [ ] 2.4 TranslationService — перевод заголовков и саммари на русский
- [ ] 2.5 Cron-скрипт `process_news.php` для пакетной обработки
- [ ] 2.6 Обновление шаблонов (вывод русских заголовков, категорий)
- [ ] 2.7 Оптимизация расхода токенов (проверка длины промптов)

#### КРИТЕРИИ ПРИЁМКИ (ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА):
1. **Проверка Скрипта:** Запуск `php scripts/process_news.php` **НЕ вызывает ошибок** и **НЕ падает** с исключением.
2. **Проверка Статуса:** В таблице `articles` должно появиться **минимум 10 статей** со статусом `processed`. Статус `new` должен уменьшаться.
3. **Проверка Контента:** Выполни SQL `SELECT title_ru, summary_ru FROM articles WHERE status='processed' LIMIT 5`. Поля **НЕ должны быть NULL**.
4. **Проверка Категории:** Поле `category_slug` для обработанных статей заполнено корректными значениями (не NULL).

**🛑 СТОП! Если эти критерии не выполнены, НЕ ПЕРЕХОДИ к Phase 3. Чини Phase 2.**

---

### PHASE 3: Кластеризация
**Цель:** Похожие новости группируются, чтобы не спамить ленту дублями.

**Задачи:**
- [ ] 3.1 ClusteringService — алгоритм определения похожести (по эмбеддингам или пересечению слов)
- [ ] 3.2 Логика создания новых кластеров и добавления статей в существующие
- [ ] 3.3 Выбор "главной" новости кластера для отображения
- [ ] 3.4 UI компоненты кластера ("Также об этом пишут...")
- [ ] 3.5 Отдельная страница кластера (список всех статей по теме)
- [ ] 3.6 Cron-скрипт `cluster_news.php`

#### КРИТЕРИИ ПРИЁМКИ (ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА):
1. **Проверка Скрипта:** Запуск `php scripts/cluster_news.php` отрабатывает корректно.
2. **Проверка Кластеров:** В таблице `clusters` создано **минимум 5 записей**.
3. **Проверка Связи:** Выполни SQL `SELECT COUNT(*) FROM articles WHERE cluster_id IS NOT NULL`. Результат должен быть > 0.

---

### PHASE 4: Фильтры и поиск
**Цель:** Пользователь может удобно искать новости.

**Задачи:**
- [ ] 4.1 API endpoint `/api/filters` (список доступных стран, категорий)
- [ ] 4.2 JS-логика фильтрации на клиенте (без перезагрузки или с ней)
- [ ] 4.3 Фильтр по странам (клики по флагам)
- [ ] 4.4 Фильтр по категориям (теги)
- [ ] 4.5 Фильтр по языку оригинала
- [ ] 4.6 Сохранение состояния фильтров в URL (History API)

#### КРИТЕРИИ ПРИЁМКИ (ФУНКЦИОНАЛЬНАЯ ПРОВЕРКА):
1. **API:** Endpoint `/api/filters` возвращает корректный JSON со списком используемых стран.
2. **UI:** При клике на фильтр (например, "Германия") список новостей обновляется.
3. **URL:** При обновлении страницы с активным фильтром состояние восстанавливается.

---

### PHASE 5: Админка
**Цель:** Ручная модерация и управление.

**Задачи:**
- [ ] 5.1 Авторизация (login/logout, сессии)
- [ ] 5.2 Dashboard со статистикой (сколько собрано, сколько обработано)
- [ ] 5.3 Список новостей на ручной модерации (status='moderation')
- [ ] 5.4 Кнопки Approve/Reject/Edit
- [ ] 5.5 Управление источниками (вкл/выкл)
- [ ] 5.6 Просмотр системных логов

**Критерий завершения:** Админ может зайти и удалить/одобрить статью.

---

### PHASE 6: SEO и продакшен
**Цель:** Сайт готов к публичному запуску.

**Задачи:**
- [ ] 6.1 ЧПУ (slugs) для новостей и кластеров
- [ ] 6.2 Meta-теги (title, description, OG) для соцсетей
- [ ] 6.3 Автогенерация sitemap.xml
- [ ] 6.4 Настройка robots.txt
- [ ] 6.5 Финальное тестирование производительности
- [ ] 6.6 Настройка GitHub Actions (CI/CD)

**Критерий завершения:** Сайт проходит валидацию SEO и готов к трафику.
